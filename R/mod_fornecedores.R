...
# R/mod_fornecedores.R ----------------------------------------------
mod_fornecedores_ui <- function(id) {
  ns <- NS(id)
  ...
  card(
    ...
    card_body(
      reactableOutput(ns("tbl_fornecedores"))
    )
  )
}

mod_fornecedores_server <- function(id, dados_filtrados) {
  moduleServer(id, function(input, output, session) {

    output$tbl_fornecedores <- renderReactable({
      df <- dados_filtrados()
      req(nrow(df) > 0)

      tab <- df %>%
        group_by(final_supplier_production, coo) %>%
        summarise(
          n_styles        = n_distinct(style_number),
          total_custo     = sum(total_custo, na.rm = TRUE),
          total_verba     = sum(total_verba, na.rm = TRUE),
          total_retail_us = sum(total_retail_us, na.rm = TRUE),
          markup_us       = ifelse(total_custo > 0, total_retail_us / total_custo, NA_real_),
          .groups = "drop"
        ) %>%
